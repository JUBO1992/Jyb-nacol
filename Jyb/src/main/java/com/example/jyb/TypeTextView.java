...
public class TypeTextView extends TextView {
	...
	private String mIntervalChar = DEFAULT_INTERVAL_CHAR; // 默认打字字符
	...
	public void start() {
		if (mShowTextString == null || mShowTextString.trim() == "")
			return;
		start(mShowTextString, TYPE_TIME_DELAY);
	}
	...
	class TypeTimerTask extends TimerTask {
		...
		public void run() {
			post(new Runnable() {
				@Override
				public void run() {
					int strlen = getText().toString().length();
					if (strlen < mShowTextString.length()) {
						setText(mShowTextString.substring(0, ++strlen) + ((strlen & 1) == 1 ? mIntervalChar : ""));
						// startAudioPlayer();
						startTypeTimer();
					} else {
						setText(mShowTextString);
